...
void setup()
{
    ...
    // Start BMS task
    xTaskCreate(BMS_task, "BMS_task", 1024 * 6, NULL, 1, &pvCreatedTask);
}
...
void BMS_task(void *param)
{
    ...
    bool flag = false;
    while (1) {
        if (foo < 9.0 || flag == true) {
            if (foo < 9.0)
                flag = true;
            foo += 0.01;
        } 
        if (foo > 15.0 || flag == false) {
            if (foo > 15.0)
                flag = false;
            foo -= 0.01;
        }
        lv_msg_send(MSG_BMS_UPDATE, &foo);
        delay(10);
    }
}