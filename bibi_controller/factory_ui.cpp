...
void ui_begin()
{
    ...
    lv_obj_t *bms_label = lv_label_create(cout);
    lv_obj_align_to(bms_label, bat_label, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);
    lv_msg_subsribe_obj(MSG_BMS_UPDATE, bms_label, NULL);
    lv_obj_add_event_cb( 
        bms_label,  
    [](lv_event_t *e) {
        lv_obj_t *label = (lv_obj_t *)lv_event_get_target(e);
        lv_msg_t *m = lv_event_get_msg(e);
        const float *v = (const float *)lv_msg_get_payload(m);
        lv_label_set_text_fmt(label, "BMS volt : %.2f V\r\n", *v);
    },
    LV_EVENT_MSG_RECEIVED, NULL);

    lv_obj_t *tpoint_label = lv_label_create(cout);
    lv_obj_align_to(tpoint_label, bms_label, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);
